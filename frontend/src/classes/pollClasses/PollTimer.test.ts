...
describe('A Conversation Area\'s Poll Timer ', () => {
    ...
    it('should be initialized correctly', () => {
        ...
        expect(testPollTimer.duration).toBe(10);
    ...
    it('should correctly count down every second', () => {
        ...
        expect(testPollTimer.duration).toBe(9);
    });
    it('should be able to add a specified number of seconds to the timer', () => {
        ...
        expect(testPollTimer.duration).toBe(20);

        jest.advanceTimersByTime(1000);
        expect(setInterval).toHaveBeenCalledTimes(1);
        expect(testPollTimer.duration).toBe(19);
        testPollTimer.addTime(10);
        expect(setInterval).toHaveBeenCalledTimes(1);
        expect(testPollTimer.duration).toBe(29);
    });
